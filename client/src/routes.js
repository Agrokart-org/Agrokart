...
const CustomerRoute = ({ children, useSidebar = false }) => {
  ...
  if (user && user.role !== "customer") {
    console.warn(
      `⛔ CustomerRoute: Role mismatch (${user.role}), redirecting...`,
    );
...
const AdminRoute = ({ children }) => {
  ...
  // Check admin access: localStorage flag OR user role OR master email
  const storedRole = localStorage.getItem("userRole");
  const adminLoggedIn = localStorage.getItem("adminLoggedIn");
  const isAdmin =
    storedRole === "admin" ||
    adminLoggedIn === "true" ||
    (user && (user.role === "admin" || user.email === "admin@agrokart.com"));
...
const Routes = () => {
  return (
    <RouterRoutes>
      ...
      <Route
        path="/delivery/assignments/:assignmentId/preview"
      ...
      />
      {/* Order Tracking (Blinkit-style real-time tracking) */}
      <Route
