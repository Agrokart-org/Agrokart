...
const VendorRegistrationPage = () => {
  ...
  useEffect(() => {
    console.log(
      "📄 VendorRegistrationPage mounted at path:",
  ...
  useEffect(() => {
    if (location.state) {
      ...
      if (email || name || phone) {
        console.log("📝 Pre-filled data from previous page:", {
          email,
          name,
          phone,
        });
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.log(
        "🔄 Starting vendor registration process with Firebase + backend...",
      );
      ...
      try {
        ...
        console.warn(
          "⚠️ Firebase sign-up for vendor failed or user exists:",
        ...
        if (firebaseError.code === "auth/email-already-in-use") {
          try {
            ...
            firebaseIdToken = await user.getIdToken();
            console.log(
              "✅ Signed in existing user successfully for vendor registration:",
              firebaseUid,
            );
          } catch (loginError) {
            ...
            if (loginError.code === "auth/wrong-password") {
              throw new Error(
                "An account with this email exists, but the password provided is incorrect. Please correct it or log in first.",
              );
            }
            throw new Error(
              "This email is already registered. Please log in to your existing account to become a vendor.",
            );
          }
          ...
          console.log(
            "ℹ️ Using existing Firebase session for vendor:",
            firebaseUid,
          );
        } else {
          throw new Error(
            firebaseError?.message || "Failed to create Firebase account",
          );
        }
      }
      ...
      const authToken =
        response.token || response.user?.token || firebaseIdToken;
      ...
      if (
        Object.keys(formData.documents).some(
      ...
      console.log(
        "✅ Vendor registration complete. User signed out for manual login.",
      );
      ...
      setError(
        error.message || "Vendor registration failed. Please try again.",
      );
    } finally {
      setLoading(false);
    }
          ...
          <Grid container spacing={3}>
            ...
            {formData.serviceAreas.map((area, index) => {
              ...
              const availableDistricts =
                area.state && locationData[area.state]
                  ? Object.keys(locationData[area.state])
                  : [];

              // Helper to get pincodes based on selected districts
              const availablePincodes = area.districts
                .reduce((acc, district) => {
                  if (
                    area.state &&
                    locationData[area.state] &&
                    locationData[area.state][district]
                  ) {
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12} md={4}>
                        <Autocomplete
                          ...
                          onChange={(_, newValue) => {
                            const newServiceAreas = [...formData.serviceAreas];
                            newServiceAreas[index].districts = newValue
                              ? [newValue]
                              : [];
                      ...
                      <Grid item xs={12} md={4}>
                        <Autocomplete
                          ...
                          onChange={(_, newValue) => {
                            const newServiceAreas = [...formData.serviceAreas];
                            newServiceAreas[index].pincodes = newValue
                              ? [newValue]
                              : [];
                            setFormData((prev) => ({
                              ...prev,
