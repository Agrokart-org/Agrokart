...
const DeliveryRegistrationPage = () => {
  ...
  React.useEffect(() => {
    if (location.state) {
      ...
      if (email || name || phone) {
        console.log("📝 Pre-filled delivery registration data:", {
          email,
          name,
          phone,
        });
  ...
  const handleSubmit = async () => {
    try {
      ...
      try {
        ...
        console.log(
          "✅ Firebase user created for delivery partner:",
          firebaseUid,
        );
      } catch (firebaseError) {
        console.warn(
          "⚠️ Firebase sign-up for delivery partner failed:",
        ...
        if (firebaseError.code === "auth/email-already-in-use") {
          try {
            ...
            firebaseIdToken = await user.getIdToken();
            console.log(
              "✅ Signed in existing user successfully for delivery registration:",
              firebaseUid,
            );
          } catch (loginError) {
            ...
            throw new Error(
              "This email is already registered. Please log in to your existing account to become a delivery partner.",
            );
          }
          ...
          console.log(
            "ℹ️ Using existing Firebase session for delivery partner:",
            firebaseUid,
          );
        } else {
          throw new Error(
      ...
      console.log(
        "✅ Delivery partner backend registration successful:",
        response,
      );
      ...
      if (
        ...
        try {
          ...
          console.warn(
            "⚠️ Delivery partner document upload failed:",
            docError.message,
          );
      ...
      setError(
        error.message ||
          "Delivery partner registration failed. Please try again.",
      );
    } finally {
      setLoading(false);
    }
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={(e) =>
                  handleInputChange("serviceRadius", parseInt(e.target.value))
                }
        ...
        return (
          <Box sx={{ textAlign: "center" }}>
            ...
            <Typography variant="body1" color="text.secondary" paragraph>
              Your registration is complete. Click submit to create your
              delivery partner account. Our team will verify your documents
              within 24-48 hours.
            </Typography>
            <Alert severity="info" sx={{ mt: 2 }}>
              You will receive a notification once your account is verified and
