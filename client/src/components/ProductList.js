...
const ProductList = () => {
  ...
  const handleImageError = (event, productIndex) => {
    ...
    const fallbackImage =
      availableImages[productIndex % availableImages.length];
    event.target.src = fallbackImage;
  };

  const getProductImage = (product, index) => {
    // Use local /images/products/ path if already set
    if (
      product.images &&
    ...
    return (
      catMap[(product.category || "").toLowerCase()] ||
              ...
              <CardContent sx={{ flexGrow: 1, p: 1.5, pb: 2 }}>
                ...
                <Box sx={{ mt: "auto", display: "flex", gap: 1 }}>
                  ...
                  >
                    ...
                    <Typography
                      sx={{ mx: 0.5, fontSize: "0.875rem", fontWeight: 600 }}
                    >
