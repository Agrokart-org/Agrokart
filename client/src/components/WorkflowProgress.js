...
const WorkflowProgress = ({ variant = "stepper", showOnlyRelevant = true }) => {
  ...
  const getCompletedSteps = () => {
    return mainJourneySteps.filter(
      (step) => workflowHistory.includes(step.key) && step.key !== currentStep,
    );
  };
  ...
  if (variant === "chip") {
    const currentStepInfo = mainJourneySteps.find(
    ...
    <Box sx={{ width: "100%", maxWidth: 600, mx: "auto" }}>
      ...
      <Stepper activeStep={activeStep} orientation="vertical">
        {mainJourneySteps.map((step, index) => {
          const isCompleted = completedSteps.some(
            (completed) => completed.key === step.key,
          );
      ...
      >
        <Typography
          ...
          sx={{ textAlign: "center" }}
        >
          <strong>{completedSteps.length}</strong> of{" "}
          <strong>{mainJourneySteps.length}</strong> steps completed
        </Typography>
      </Box>
    </Box>
  );
