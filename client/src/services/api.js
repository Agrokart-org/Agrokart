...
// Wrapper to use Native HTTP on mobile (bypasses CORS/Cleartext issues)
export const safeFetch = async (url, options = {}) => {
  // Special handling for FormData (File Uploads)
  // Standard fetch handles multipart boundaries better than CapacitorHttp bridge
  // valid now that Cleartext traffic is enabled in AndroidManifest
  if (options.body instanceof FormData) {
    console.log("[Native] Using standard fetch for FormData upload");
    return fetch(url, options);
  }

  if (isMobile()) {
    try {
      console.log(`[Native] ${options.method || "GET"} ${url}`);
      ...
      console.log(`[Native] Response: ${response.status}`);
...
// DEBUG: Alert the API URL on app start (Remove in prod)
setTimeout(() => console.log(`API URL: ${API_BASE_URL}`), 2000);
  ...
  try {
    ...
    if (credentials.expectedRole) {
      if (result.user && result.user.role !== credentials.expectedRole) {
        throw new Error(
          `Access denied. This account is registered as ${result.user.role}, not ${credentials.expectedRole}.`,
        );
      }
    }
    ...
    if (
      credentials.email === "demo@shetmitra.com" &&
...
export const vendorLogin = async (credentials) => {
  try {
    ...
    if (result.user && result.user.role !== "vendor") {
      throw new Error(
        `Access denied. This account is registered as ${result.user.role}, not vendor. Please use the correct login page for your account type.`,
      );
    }
...
export const deliveryLogin = async (credentials) => {
  try {
    ...
    if (result.user && result.user.role !== "delivery_partner") {
      throw new Error(
        `Access denied. This account is registered as ${result.user.role}, not delivery partner. Please use the correct login page for your account type.`,
      );
    }
    ...
    throw new Error(
      error.message ||
        "Delivery partner login failed. Please check your credentials.",
    );
  }
};
...
export const getProduct = async (id) => {
  try {
    ...
    console.log("Backend fetch failed, checking mock data");
  }

  // Return mock data fallback (only matches if ID exists in mock)
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if (product) {
        ...
        // If we tried API and it returned 404, OR if network failed,
        // AND not in mock data, then truly not found.
...
export const getFeaturedProducts = async (limit = 10) => {
  try {
    ...
    console.log(
      "Backend not available, using mock featured products:",
      error.message,
    );
  }
...
export const verifyPickup = async (orderId, pin, token) => {
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(errorData.message || "Pickup verification failed");
  }
...
export const deliveryRegister = async (data) => {
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(
      errorData.message || "Delivery partner registration failed",
    );
  }
...
export const createPaymentOrder = async (amount, token) => {
  try {
    ...
    const response = await safeFetch(`${API_BASE_URL}/payment/create-order`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-auth-token": token,
...
export const verifyPayment = async (data, token) => {
  ...
  const response = await safeFetch(`${API_BASE_URL}/payment/verify-payment`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "x-auth-token": token,
...
export const getAvailableAssignments = async (token) => {
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(
      errorData.message || "Failed to fetch available assignments",
    );
  }
...
export const rejectAssignment = async (id, token) => {
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(errorData.message || "Failed to reject assignment");
  }
