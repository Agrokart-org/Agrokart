...
class ProductsService {
  ...
  async init() {
    ...
    try {
      ...
      console.log(
        "🌾 ProductsService initialized from API:",
  ...
  /**
   * Fetch products from API (force refresh)
   */
  async fetchProducts() {
    await this.init();
  ...
  /**
   * Get all products (sync if already initialized, otherwise returns empty/promise logic needs handling in caller)
   * Better to use fetchProducts() in components
   */
  getAllProducts() {
    return this.products;
  }
  ...
  getStatistics() {
    ...
    const avgRating =
      this.products.reduce((sum, p) => sum + (p.rating || 0), 0) /
      totalProducts;
