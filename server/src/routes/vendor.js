...
// Verify Pickup by Delivery Partner
router.post("/orders/:orderId/verify-pickup", auth, async (req, res) => {
  try {
    ...
    // 1. Verify Vendor exists & has role
    const vendor = await User.findById(vendorId);
    if (!vendor || vendor.role !== "vendor") {
      return res.status(403).json({ message: "Access denied" });
    }

    // 2. Fetch the delivery assignment for this order & vendor
    const assignment = await DeliveryAssignment.findOne({
      order: orderId,
      vendor: vendorId,
      status: "accepted", // Ensure a delivery partner is on it and hasn't picked up yet
    ...
    // 3. Check the PIN
    if (assignment.vendorPickupPin !== pin) {
      return res
        .status(400)
        .json({
          success: false,
          message:
            "Invalid PIN. Please ask the delivery partner to show their pickup PIN.",
        });
    }

    // 4. Verification Successful -> Update Assignment and Order
    assignment.status = "picked_up";
    assignment.pickupTime = new Date();
    assignment.tracking.status = "picked_up";
    assignment.tracking.updatedAt = new Date();
    await assignment.save();

    const order = await Order.findById(orderId).populate(
      "items.product",
      "name price images unit category",
    );
    if (order) {
      order.orderStatus = "out_for_delivery";
      await order.save();

      // Stock is already deducted at order creation time (in orders.js),
      // so we only release the reservedStock here (no double-deduction).
      for (const item of order.items) {
        if (item.vendor && item.vendor.toString() === vendorId.toString()) {
          ...
          if (inventoryItem) {
            ...
            await inventoryItem.save();
            console.log(
              `📦 Stock confirmed (already deducted at order time): ${item.product.name || item.product} qty=${item.quantity}, remaining=${inventoryItem.stock}`,
            );
          }
        }
      }
      ...
      if (assignment.deliveryPartner) {
        io.to(`delivery_partner_${assignment.deliveryPartner}`).emit(
        ...
        // Fetch real delivery partner user doc if available
        const dp = await User.findById(assignment.deliveryPartner);
        if (dp && dp.firebaseUid) {
          io.to(`delivery_partner_${dp.firebaseUid}`).emit(
            "delivery_status_updated",
            {
              assignmentId: assignment._id,
              status: "picked_up",
      ...
      await Notification.createNotification({
        ...
        message: `Your order ${order.trackingNumber} has been picked up by the delivery partner.`,
        data: {
          orderId: order._id,
          actionUrl: `/orders/${order._id}`,
        },
    ...
    res.json({
      success: true,
      message: "Pickup verified successfully. Hand over the items.",
      order: order, // Return order so frontend can highlight items to hand over
    });
  } catch (error) {
    console.error("Verify pickup error:", error);
    // Log to physical file for debugging
    const fs = require("fs");
    const logMsg = `[${new Date().toISOString()}] VERIFY_PICKUP_ERROR: ${error.message}\n${error.stack}\n`;
    try {
      fs.appendFileSync("orders_debug.log", logMsg);
    } catch (e) {}
    res.status(500).json({ message: "Server error", details: error.message });
  }
});
...
router.post("/orders/:orderId/claim", auth, async (req, res) => {
  try {
    ...
    // 2. verify Stock Availability AGAIN (Crucial step)
    // We assume the whole order is taken by one vendor for this flow
    ...
    for (const item of order.items) {
      ...
      // Reserve/Deduct logic
      // Directly reserve here
      inventoryItem.reservedStock += item.quantity;
      await inventoryItem.save();
    ...
    if (vendor.address && vendor.address.coordinates) {
      ...
      // BROADCAST FALLBACK: Send to ALL connected sockets
      console.log(
        "BROADCAST FALLBACK (claim): Emitting delivery_request & new_assignment to ALL sockets",
      );
      io.emit("new_assignment", claimDeliveryNotificationData);
      io.emit("delivery_request", claimDeliveryNotificationData);
    }
    ...
    io.emit("order_no_longer_available", {
      orderId: order._id,
      reason: "claimed",
    });

    // Also emit to the specific vendor who claimed it (for confirmation/UI update)
    io.to(`vendor_${vendor._id}`).emit("order_claimed_success", {
      orderId: order._id,
      order: order,
    });
