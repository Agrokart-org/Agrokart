...
const deliveryAssignmentSchema = new mongoose.Schema(
  {
    ...
    deliveryPartner: {
      type: mongoose.Schema.Types.ObjectId,
      ref: "User",
      required: false, // Changed to false for open pool assignments
    },
    rejectedBy: [
      {
        type: mongoose.Schema.Types.ObjectId,
    ...
    vendorPickupPin: {
      type: String,
      required: false, // Generated when assignment is accepted/created
    },
    tracking: {
      currentLocation: {
        type: {
...
deliveryAssignmentSchema.virtual("deliveryDuration").get(function () {
  if (this.actualPickupTime && this.actualDeliveryTime) {
    return Math.round(
      (this.actualDeliveryTime - this.actualPickupTime) / (1000 * 60),
    ); // in minutes
  }
  return null;
});
...
deliveryAssignmentSchema.methods.calculateDistance = function () {
  if (
    this.pickupLocation.coordinates.coordinates.length === 2 &&
...
deliveryAssignmentSchema.methods.updateLocation = function (
  coordinates,
  speed = 0,
  heading = 0,
) {
