...
earningsSchema.virtual("totalDeductions").get(function () {
  ...
  return (
    (this.deductions.platformFee || 0) +
    (this.deductions.processingFee || 0) +
    (this.deductions.penalty || 0) +
    (this.deductions.other || 0)
  );
});

// Pre-validate middleware to calculate net amount before mongoose checks required fields
earningsSchema.pre("validate", function (next) {
  // Calculate commission amount
  this.commissionAmount = (this.grossAmount * (this.commissionRate || 0)) / 100;

  // Calculate tax amount
  this.taxAmount = (this.grossAmount * (this.taxRate || 0)) / 100;
...
earningsSchema.statics.getEarningsSummary = async function (
  userId,
  userType,
  period = {},
) {
...
earningsSchema.statics.getMonthlyTrend = async function (
  userId,
  userType,
  year,
) {
